...
public class Giocatore implements Serializable
{
    ...
    public boolean esamina(String oggetto)
    {
        ...
        if (oggetto.equals("luogo"))
        { // se viene richiesto di esaminare il luogo
        ...
        else
        {
            ...
            { // si effettua una ricerca fra gli oggetti
                // presenti nel luogo
                ...
                if (item.getNome().equals(oggetto) && item.isVisibile())
                {
                    ...
                    { // se l'oggetto esaminato è un contenitore
                        Contenitore a = (Contenitore) item; // viene convertito in un contenitore(cast)
                        for (Oggetto u : a.getOggetti())
                        { // e tutti gli oggetti che contiene diventano visibili
            ...
            if (esito == false)
            { // se la ricerca nel luogo non ha dato esito positivo
                for (Oggetto item : this.getInventario())
                { // si cerca l'oggetto nel proprio inventario
    ...
    public boolean raccogli(String oggetto)
    {
        ...
        for (Oggetto item : this.getLuogoAttuale().getOggetti())
        { // se l'oggetto è presente nel luogo
            if (item.getNome().equals(oggetto) && item.isVisibile() && item.isPrendibile())
            { // se è prendibile
    ...
    public boolean lascia(String oggetto)
    {
        ...
        for (Oggetto item : this.inventario)
        { // si ricerca l'oggetto nell'inventario
            if (item.getNome().equals(oggetto))
            { // se lo si trova
        ...
        try
        {
            ...
            if (l != null && ((!l.isBloccato()) || (haOggetto(l.getOggettoSbloccante().getNome()))))
            {
                ...
                { // se il luogo è bloccato
                    ...
                    for (Oggetto item : this.inventario)
                    { // si ricerca nell'inventario la chiave
    ...
    public boolean usa(String oggetto)
    {
        ...
        for (Oggetto item : this.getInventario())
        { // si cerca l'oggetto da usare nell'inventario
            if (item.getNome().equals(oggetto))
            {
                if (item instanceof Arma)
                { // se è un'arma la si equipaggia
        ...
        {
            for (Oggetto item : this.getLuogoAttuale().getOggetti())
            { // se l'oggetto si trova nel luogo
                if (item.getNome().equals(oggetto))
                {

                    if (item instanceof Utilita && item.isUsabile())
                    { // ed è usabile
    ...
    public boolean attacca(Nemico nemico)
    {
        ...
        if (this.getArmaInUso() != null)
        { // se ho un'arma equipaggiata attacco il nemico
